...
<urlset
    ...
    <url>
        <loc>https://www.nuevamedicina.online/tienda</loc>
    ...
    <url>
        <loc>https://www.nuevamedicina.online/tienda/decoracion</loc>
        <lastmod>{{ date('c', time()) }}</lastmod>
        <changefreq>daily</changefreq>
    ...
    <url>
        <loc>https://www.nuevamedicina.online/tienda/remodelacion</loc>
        <lastmod>{{ date('c', time()) }}</lastmod>
        <changefreq>daily</changefreq>
        <priority>0.90</priority>
    </url>
            ...
            @if($parentCat->is_parent)
                ...
                @foreach($parentCat->categories()->ordered()->get() as $childCat)
                    @if($childCat->products->count() >= 1)
                        <url>
                            <loc>https://www.nuevamedicina.online/tienda/categoria/{{ $parentCat->slug }}/{{ $childCat->slug }}</loc>
                        ...
                        @foreach($childCat->products as $product)
                            <url>
                                <loc>https://www.nuevamedicina.online/tienda/{{ $product->vendor->slug }}/{{ $product->slug }}</loc>
                ...
                @endforeach
                <url>
                    <loc>https://www.nuevamedicina.online/tienda/categoria/{{ $parentCat->slug }}</loc>
    ...
    @foreach($vendors->all() as $vendor)
        @if($vendor->products->count() >= 1)
            <url>
                <loc>https://www.nuevamedicina.online/tienda/{{ $vendor->slug }}</loc>
